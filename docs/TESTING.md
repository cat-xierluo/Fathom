...
仓库的三个 fail-closed 入口会建立/核对各自环境和精确通过数；本地 pytest 配置门禁为 **696**（ISS-030A +16、ISS-040C +14、ISS-037 repair3 +11：`tests/test_third_party_notices.py` notices 许可证值级核对），CI 与 `scripts/ci_pytest.sh` 已同步为 **696**（2026-09-23 ISS-037 repair3），变更测试数量必须用新任务和反例显式同步。本地复跑使用 macOS 系统 Bash：
...
```bash
/bin/bash scripts/ci_pytest.sh                       # ↔ pytest (arm64)，断言 696（ISS-037 repair3 同步）
