...
- 运行根、扫描根、资源根和端口可显式隔离；旧开发库打开时会经 schema v2 校验、WAL 一致备份和事务迁移。
...
- 应用图标与菜单栏 tray 为正式「深度环」品牌资产（ISS-045，2026-09-19 实测 DMG 安装窗口/Dock/菜单栏渲染）；图标未验证项：18pt 小菜单栏下 tray 可辨性（Launchpad/Spotlight 检索呈现与深浅色模式已于 2026-09-19 实测）。其余未验证项：无 Python/Rust/Homebrew 新账户从 DMG 首启、tray 菜单手点退出、断网首启（握手页实机渲染已于 2026-09-18 验证）。
