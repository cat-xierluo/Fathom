...
### [DEC-020] - 2026-09-19 - 外部 PR 审查采用「必要性 gate → 代码审查」两层顺序，本地验证一律隔离 worktree

**背景**

审自己的 PR 时两个前提默认成立：主工作区干净、需求来自项目路线图。审外部贡献者的 PR 时两者都可能不成立——工作区可能残留上一轮审查的 merge/rebase 中间态，PR 本身可能只是贡献者的个性化需求。Folia #165 实战双重踩坑：(1) 先代码深审发现多个缺陷（数据丢失链、确认框「保存」不兑现），最终在「项目要不要」层面以伪需求否决，代码审查全部沉没；(2) 主工作区残留审 #166 时中断的 merge，`gh pr checkout` 把遗留变更叠加成混血工作区，typecheck 假阳性险些误判 #165 不合格。

**决策**

外部 PR 审查固定两层顺序：第一层项目契合度 gate（需求来源核对、README 核心主张判据、伪需求识别），不通过直接礼貌关闭、不投入代码审查；第二层代码审查（diff 范围、隔离 worktree 本地验证、正文最低要求、Fail-Closed 合并门禁）。本地验证一律 `git worktree add` 隔离检出，主工作区动手前体检、收尾清理中间态；报错文件不在 PR diff 内视为污染信号。必要性判断的依据（ROADMAP、issue、核心主张）是项目级答案，Skill 只约束流程顺序，答案缺失时问用户或查项目规则。

**影响**

- `git-workflow` 升级为 v1.8.7（v1.8.6 环境卫生、v1.8.7 分层审查）。
- gate 不通过时不再产生代码级审查结论，切断沉没成本路径。
- review 结论可信度锚定「报错与 PR diff 对不上先怀疑工作区污染」，跨 PR 接力审查的假阳性被显式规则拦截。

### [DEC-019] - 2026-09-05 - 分支清理采用独立 reference 作为单一权威

**背景**

`SKILL.md` 同时承载分支创建、长期集成、单 Worker 自动清理、批量 stale 审计、PR、冲突、提交和多 Worktree 规则。清理部分既长，又与 `multi-agent-orchestration`、长期集成分支 reference 重复，容易让生命周期判定与删除命令在多处漂移。

**决策**

主文档只保留生命周期分流和必须加载的入口；新增 `references/branch-lifecycle-and-cleanup.md` 作为清理权威，统一一次性 Worker、长期功能线、单任务自动清理、批量审计与关闭授权。`references/long-lived-integration-branch.md` 继续负责建线、同步和里程碑，不重复展开通用清理流程。

**影响**

- Git 主文档减少约百行默认上下文，但删除授权与 fail-closed 边界不变。
- `multi-agent-orchestration` 只引用 Git 清理权威，不再复制批量审计细节。
- 后续修改生命周期或删除规则时，必须先更新本 reference，再同步主文档的摘要与版本记录。

### [DEC-018] - 2026-09-05 - 分支是否可清理由生命周期决定，而不是由“已经合并”决定

**背景**

长期功能/集成分支可能接收多个已验收 Worker PR，并在里程碑后继续作为下一波基线。它与一次性 Worker head 都可能呈现“已经合并”，因此仅按 merge 状态自动清理会混淆分支角色。

**决策**

把 `integration_target` 与 Worker `head` 分开建模，并持久声明 `branch_lifecycle`。单任务自动清理只授权删除 `ephemeral-worker` head；`long-lived` 分支及其固定 Worktree 不进入自动或常规 stale 批量清理。PR base、metadata base 和调用目标必须一致，生命周期不得由调用方降级。

**影响**

- 短 Worker 合入长期功能分支后，短分支照常清理，长期目标继续保留。
- 长期分支即使已合入默认主干，也要等功能线明确关闭、未决工作处置完毕并取得独立删除授权。
- `multi-agent-orchestration` v2.16.1 提供机械实现与防绕过回归。

### [DEC-017] - 2026-09-05 - 验收后的 worker 资源采用 delivery-bound 自动清理

**背景**

PM 已验收并交付 worker 任务后，经常漏删短期分支与 worktree；依靠事后批量审计既积累本地资源，也让项目视图持续出现已经完成的工作。另一方面，squash/rebase merge 下原分支 tip 不可达 main，`git branch -d` 会拒绝，直接改用 `-D` 又会丢失身份漂移或未推送工作的保护。

**决策**

单 worker 收口默认紧随交付执行清理，并把 PR、head branch、40 位 tip、delivery commit、worktree 与 lifecycle 绑定为同一证据包。远端事实或 tip 查询失败、dirty/active 状态、身份漂移均失败关闭。worktree 移除后，本地 ref 仅允许以 expected tip 为 old-value 精确删除；清理终态固定为 `CLEANED`、`RETAINED_WITH_REASON`、`CLEANUP_PENDING`。

**影响**

- 自动清理是任务资源闭环的默认组成部分；显式保留必须带理由。
- 已确认的交付与后续清理债务分开记账，清理失败不得触发 push/merge 盲重试。
- 批量 stale 分支清理仍须单独展示候选并取得用户确认，不因单任务协议而扩大删除权限。
...
- **目标:** 把 Folia 四 PR 接力审查（#165/#166）暴露的两类外部 PR 审查缺陷固化为流程规则。
- **操作:** SKILL.md §4 新增「分层审查——必要性 gate 先于代码审查」与「本地验证的环境卫生」两小节；版本 1.8.5→1.8.7；CHANGELOG 补 [1.8.6]/[1.8.7]；DECISIONS 补 DEC-020；TASKS 补 Task-009 完成记录。
- **结果:** 外部 PR 审查顺序固化为 gate → 代码两层，本地验证强制隔离 worktree；实战来源 Folia #165（伪需求深审沉没）与 #166 遗留 merge 叠加（typecheck 假阳性）。
- **下一步:** 无；TASKS 队列维持 Task-001~005 DRAFT 不变。
