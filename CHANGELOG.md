...
- 精确 pytest 门禁 179→209→260（ISS-024 新增 30 项查询口径测试、ISS-047 新增 21 项瞬时错误分类测试）；原 179→209（新增 30 项差分语义测试，全部合成隔离数据）；安全浏览器夹具造数改用生产默认阈值口径，与 API 扫描同数据集；前端模块/生命周期检查扩至 33 项真实 Chromium 用例。
