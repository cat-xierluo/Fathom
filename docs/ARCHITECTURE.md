...
| GET | /api/volume-trend?limit= | 以最新快照为锚取最新 N 条后正序输出；按数据集 (root, min_kb) 隔离，跨根/跨阈值不混点 |
