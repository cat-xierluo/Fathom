...
更新：2026-09-12。用户当前任务为 ISS-017；当前实施阶段 M0。产品方向见 [ROADMAP](ROADMAP.md)，页面合同见 [DESIGN](DESIGN.md)，验证方法见 [TESTING](TESTING.md)。编号永久保留，不复用。
...
审查基线 main=`33e81f9`。另一个会话正在 `integration/wave1` 集成 ISS-003/007/008；本规划不替它合并或宣称验收。发布到私有远端只上传已核验主干和本规划分支；其他分支按其所属会话流程处理。
...
- **验收**：
  - [ ] 指定临时 runtime/root/port 后 API/CLI 的所有读写均留在指定范围
