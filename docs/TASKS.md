...
- **最新合并与在途**：ISS-080 由 #150、Wave39 回写 #151、ISS-040C 由 [PR #152](https://github.com/cat-xierluo/fathom/pull/152) 合并为 `696f3b3`（合并后 main 全量：pytest 685/cargo test 57/版本一致 ok，前端零改动故浏览器检查未复跑）。2026-09-23 两轮手动 PM 会话（用户直接指令）完成派发与收口；期间 Orca runtime 中途重启一次（a85a023e→be2406ff），040C reviewer 的 worker_done 送达通道断裂（ORCA_COMPLETION_AUTHORITY_INVALID，worker 按协议停止留档），PM 以产物验收并披露；该事件也解释了 cron 心跳 20 轮零动作（coordinator 绑定随重启失效）。PM 心跳已随本轮收口恢复（当前为待命只读 cron，每 20 分钟零副作用简报）。**2026-09-23 21:45 前后 Wave41 手动 PM 会话接管**（用户直接指令：确认 Actions 额度恢复、要求尽快推进 GitHub Actions 发版、license/开源声明参照 Folia 项目）：CI workflow 曾按 DEC-021 恢复命令重新启用（state=active），但 PR #154/#155 触发的 4 个 run 全部 job 在执行任何步骤前被拒、无日志——**额度实测未恢复（NOT_RUN）**，CI 已再次停用止损（详见 DEC-024 验证段）；ISS-037 经用户授权解除暂停、按 DEC-024 Folia 基线重派 repair3；登记并派发 ISS-041A（arm64 未签名 draft Release CI，额度恢复即可用，实现不受阻断）。#154 已登记 ISS-081。**2026-09-23 22:0x 第二轮手动 PM 会话按用户「合并这些 PR」指令收口**：#156（ISS-037 repair3，`2b70686`）/#157（ISS-041A 代码，`bb6ac2c`）/#158（ISS-081，经 PM rebase 解计数冲突后合并，`a7cc984`）三 PR 均经 PM 核验（037 audit+METADATA 复验、041A 静态安全核验、081 满盘全量复跑）合并，合并例外（无独立 reviewer，用户指令+PM 复验）已在各卡登记；main 终态门禁 **697 passed** + 版本一致；七个已合并远端分支与六个 worktree 清理完毕（远端只剩 main），iss-074 分支独有 8 行补记存档 `.git/orchestration/cleanup-20260923/`。**22:3x–22:4x Wave41 会话（用户确认另一会话终止后由本会话接手）继续收口**：041A 独立 reviewer 对 fixed head `2aa340d` 返回 **REJECT（B-1：干净 runner venv 链断）**——收口时点未知的时序性事实，已补登 041A 卡「证据/接续补充」段并修正「无独立 reviewer」表述；repair1（[PR #160](https://github.com/cat-xierluo/fathom/pull/160)，`FATHOM_BUILD_VENV` 透传 + `--target` 修正）经修复轮独立 reviewer **ACCEPT 0 blocking** 后合并为 `2195e0d`，main 全量 **697 passed**。041A 代码侧就绪，WAITING 等额度/公开决策与 tag 首跑。
- **当前接续顺序（2026-09-23 22:4x 更新）**：①ISS-041A 首跑（WAITING：B-1 已修复合并 #160、代码侧无已知阻断；等 Actions 额度/公开路径决策 + 用户定 tag 后由 PM 在 `v*` 执行 draft Release 实证并回勾）；②G10 安装升级实测与生产 updater keypair 决策（用户门，PM 亲自执行不自动）；③按条件补 ISS-028/077 前台验证与 ISS-002/003/008/009 的同候选实测，ISS-010/016 的 G8/G9 在隔离测试账户完成；④ISS-041 父卡余项（Intel 双架构、updater 产物与 latest.json、publish 人工门），最后 ISS-030 实包恢复、ISS-033 外部验收。ISS-037/081 已 DONE、代码队列无 READY；没有合法 READY 时报告具体缺口，不以重复文档/夹具或视觉小修维持派发。
...
- 上下文回写须检查摘要、索引、当前卡头是否一致；功能变化同步 README/ARCHITECTURE，用户可见变化同步 CHANGELOG；DESIGN/TESTING/被引用实测清单的“未实现”声明须随代码事实更新。历史证据保留日期，不再用追加流水制造第二条当前队列。
- 夹具只证明其实际调用的生产模块与所注入的故障；自建 fake 升级器通过不能证明 `updater_install` 已安全接入。每张切片明确“已实现 / 未接入 / 已实测 / NOT_VERIFIED”及唯一父卡；不得用测试总数、子卡数或“已合并”替代发行退出条件。
...
- **后继查表**：以当前接手摘要和索引依赖为准。父卡存在人工门时，可登记有完整范围与验收的独立代码/验证准备切片；不得把父卡 DONE 当作领取准备工作的必要条件，也不得把切片完成反写成父卡已验收。
...
无分支限定的 ISS-017/018 以本索引“全项目审查与规划/拒绝无效扫描”为准。本轮已按上表迁移 wave1 文档；以后引用旧工作时仍按此映射，保留原始提交中的引用，不以旧任务文件覆盖本任务源。最新编号见下方索引；子卡保留原编号后缀，不复用历史编号。

## 索引

| ID | 任务 | 优先级 | 阶段 | 状态 | 前置 |
|---|---|---|---|---|---|
| ISS-001 | 真实跨日定时日报验证 | P1 | M1 | DONE | — |
...
| ISS-028 | 总览、变化与目录详情 UX/UI 实装 | P1 | M1 | WAITING | ISS-021、ISS-024、ISS-026、ISS-027 |
...
| ISS-041A | arm64 未签名 draft Release 工作流与候选校验（ISS-041 首轮切片） | P1 | M2 | WAITING | ISS-031 |
| ISS-042 | 修复父子变化折叠的 topn 提前截断 | P0 | M0 | DONE | — |
| ISS-043 | 同步 pytest 精确数量门禁 | P0 | M0 | DONE | — |
| ISS-044 | 同步扫描协调后的 pytest 精确门禁 | P0 | M0 | DONE | ISS-020 |
| ISS-045 | Fathom Logo 与应用图标资产（深潭 App 图标 + 深度环界面形态） | P1 | M1 | DONE | — |
...
| ISS-030A | 升级协调协议夹具：N→N+1 停写/备份/替换/握手自动验（ISS-030 代码切片，076 清单 F1） | P1 | M2 | DONE | ISS-025、ISS-040B |
...
| ISS-081 | 通知语义测试隔离：固定空间告警输入，满盘机器门禁不假红 | P1 | M0 | DONE | — |
...
- **状态**：DONE（P1/M0，2026-09-23；[PR #158](https://github.com/cat-xierluo/fathom/pull/158) squash 合并为 `a7cc984`，worker head 经 PM rebase 收敛后为 `44bb80b`——原 head 基于 #155 时代基线，与 #156 计数行冲突，PM 按职责 rebase 解冲突（686 语义并入 696 → **697**）并以 --force-with-lease 更新）。
...
- **验收**：
  - [x] 满盘机器全绿 —— worker 在宿主剩 9 GiB（低于阈值）时全量 686 passed（其基线口径）；PM rebase 后满盘本机全量 **697 passed**、`ci_pytest.sh` exit 0（697=expected 697）
  - [x] 隔离承重证明（结构性红绿）—— worker 负向探针：conftest 模拟 statvfs 剩 9.5GiB，隔离在位类内 4 passed；临时移除隔离固定 → 原两用例 2 failed（断言原文复现）→ 恢复。新增 `test_low_free_volume_swaps_title_to_alert_with_sound` 显式覆盖低剩余→TITLE_ALERT+声音+「剩余 5.0 GB」正文真实链路，未丢覆盖
  - [x] 生产代码零改动（diff 无 fathom/ 变更，无需 test seam）；计数同步四处（697）；TASKS 未被 worker 触碰
- **证据/接续（2026-09-23 收口）**：修复方式=类级 fixture `monkeypatch scanner._volume_stat` 受控充裕输入（500/400 GiB），statvfs→volume_stats→notify 生产链路照常运行；未采用关闭告警或改 FREE_ALERT_GB（会掩盖真实行为）。PM 验收：rebase 解冲突四处计数行（697）、满盘全量 697/门禁绿/patch-id PASS、合并 #158、main 终态门禁 697+版本一致。合并例外登记：本 head 无独立 reviewer（交付来自 Wave41 会话 worker，用户明确指令合并；负向探针证据+PM 复验承担审查责任）。
...
- **来源/目标**：用户 2026-09-22 要求检查近期变化是否跑偏，并修正供后续 Agent 使用的上下文。固定审查 `e033ef6..82b0616`，在已有 ISS-074/075 审计上复查近期接续，不重做旧实现。
- **范围**：AGENTS、TASKS、README、ARCHITECTURE、DESIGN、ROADMAP、TESTING、CHANGELOG、ISS-076 实测准备清单与本次审查证据 `docs/plans/2026-09-22-context-convergence-audit.md`。
- **边界**：仅修正文档事实、任务状态与接续边界；不写业务代码、不替其他 PM 审核/合并在途实现、不操作生产数据/服务/前台、不恢复自动化、不更改产品或发行验收标准。
- **验收**：
...
- **证据**：独立 worktree `/private/tmp/fathom-context-20260922`，分支 `iss-079-context-convergence`，基线 `82b0616c557a42961c07cd9344bd0e64e2a9c6ab`。审查期间主干前进至 `31396a08ed90a37f787614d6f32024e30836c3aa`，已保留 #146/#147 的夹具、验收和命令入口并重放本次修订；新增 CI 数字分叉已登记 ISS-080。原始文档体检限制见本次审查报告；独立 reviewer `/root/context_scan` 对最终同步候选 `7ce3d73795716b5b2811877cc83caf59ec0b3ef6`（base `31396a0`）复核 **ACCEPT、0 blocking**：030A 成果保留、生产接线缺口准确、未新增授权/降低验收。10 份 Markdown、84 个相对链接/锚点、89 组索引/卡片配对、依赖无环与围栏检查通过，`git diff --check origin/main...HEAD` 通过；后续仅追加本验收记录与 PR 指针。修订见 [PR #148](https://github.com/cat-xierluo/fathom/pull/148)，已按用户本轮明确 review/合并授权 squash 合并为 `f8a64b18d82386f6dc0ba9cf92b111dfc674c148`；本卡不代替 ISS-080 或各发行父卡的实施/验收。业务全量、GUI、打包、生产观察均 `NOT_VERIFIED`，没有复用为本次实跑。

- **合并复核（2026-09-22）**：最终 head `9ca94d8be20c66ae5b66d0bc1618118633c76ad6` 已有独立 ACCEPT、0 blocking；本轮再次检查 10 份文档、84 个相对链接、89 组索引/卡片、依赖与差异全部通过。GitHub 返回 merged=true，合并提交已进入 origin/main，且 `git diff --quiet 9ca94d8 f8a64b1` 通过（合并树与验收候选一致）。CI 只读确认为 `disabled_manually`，未恢复；业务/实机未重跑。ISS-080 与各发行父卡状态保留，本次只回写完成事实。

### ISS-080 · 同步 ISS-030A 后本地与云端 pytest 计数

- **状态**：DONE（P1/M0，2026-09-23；[PR #150](https://github.com/cat-xierluo/fathom/pull/150) squash 合并为 main `e581cdf2063d7d32ff5fa40dfb33b8d75a949213`，worker head `3bca4bc3ed28abe0722aca0f4656dca9c4e30ca8`）。审查方式：单文件配置同步（+5/−4、零生产逻辑与测试语义），按 ISS-058/027 波先例由 PM 直接审阅替代独立 reviewer；PM 未改任何业务文件。
- **来源/目标**：ISS-079 复审中主干新增 #146/#147；`31396a0` 的 `scripts/ci_pytest.sh` 默认 671，但 `.github/workflows/ci.yml` 的 `EXPECTED_PYTEST_PASSED` 与步骤名仍 655。CI 当前停用不代表配置可保持分叉，恢复后会误拒绝正确通过数。
- **范围**：`.github/workflows/ci.yml` 的 pytest 计数说明、环境值和步骤名，以及本卡/TESTING 的同步记录。不得修改测试断言、减少测试或恢复 CI。
- **实施边界**：先刷新最新 main；以已合并测试集合和本地门禁为依据，核对新增 16 项后的总数，再同步云端同口径值。ISS-030A 的测试与既有通过记录保留。
- **验收**：
  - [x] 本地默认值、云端环境值与步骤名一致，保留与新增 16 项对应的可复核依据 —— env `EXPECTED_PYTEST_PASSED: "671"`（ci.yml:44）、步骤名「pytest（断言 671 项通过）」（ci.yml:80）、注释与 `ci_pytest.sh` 同口径：671 = 655 + 16（ISS-030A），注明 655 已含 ISS-040B +8、依据 #146/#147 收口 PM 全量复跑 671 passed
  - [x] 差异仅为计数/说明同步，不修改产品、测试语义、CI 启停或权限 —— diff 仅 `.github/workflows/ci.yml`（+5/−4）；触发/权限/`concurrency` 未动；CI 保持 `disabled_manually`（DEC-021）
  - [x] 固定候选经独立复核；验证依据区分收集数量与实际通过数 —— PM 在 worktree 独立复跑三条冻结 grep 全 exit 0；通过数为 #146/#147 记录的实际通过数（PM 全量复跑 671 passed），本次未运行 `--collect-only`、无收集数冒充通过数
- **证据/接续**（2026-09-23，Wave39 手动 PM 会话）：worker `iss-080-r1`（claude-code/GLM lane；dispatch `ctx_1844fbe4db55`、run `run_0645af930625`）交付分支 `iss-080-ci-pytest-sync`，safe-push 校验通过。PM 验收链：派发价值门 ok → worker-value-postflight ok（资产匹配、无越界文件、3/3 验证证据、head 一致）→ pr-audit decision=create（无重复 PR）→ #150 squash 合并核实 merged=true。合并后 main 静态核对仅 ci.yml 9 行；无测试/代码文件变化，全量 pytest 未复跑（无新信号，671 依据 #147 记录）。云端 job 仍 `NOT_RUN`（CI 停用态），恢复后即为同口径。PM 心跳本轮切待命（备份 `.git/orchestration/cron-standby-20260923/`），本卡由用户直接指令的手动 PM 会话派发收口。
...
- **范围**：本文件、README、ARCHITECTURE、ROADMAP、TESTING、DECISIONS 的事实校正，发行方案的生效范围标注与许可证选项的坏链接修复，以及 `docs/plans/2026-09-19-pm-progression-audit.md`。
- **边界**：不改业务代码、不合并他人 PR、不接管 PM 自动化、不触发生产扫描、不更改生产服务。保留原始审查与实验记录，附更正而非伪造历史 ACCEPT 或通过证据。
- **进度/验收**：
  - [x] 固定 Git 基线、全部 worktree 和开放 PR，核对最近变化与已有任务来源
...
- **目标**：消除“索引 DONE、验收框未勾、正文又说未执行”的歧义；让新 PM 能凭仓库与 PR 复核交付边界。
- **范围**：TASKS 中已合并卡片的证据及必要的定向验证；优先 ISS-002A/067/068、ISS-059/009、ISS-069、ISS-040A、ISS-070、ISS-037，再处理审查报告列出的其余卡片。
- **实施边界**：先按审查基线列出 15 张待对账卡，再刷新 main。对每个未勾项区分：已存在可复查证据／已明确迁移给父卡／仍未验证。引用 PR 与固定 head；只有私有 session 路径的证据须补匿名摘要，不能要求下一 PM 安装旧编排工具。REJECT 项须由独立 reviewer 对修复后的组合结果复核，不得以“非本卡引入”自行降级。若确有未完成且无已登记承接卡，重开原卡或登记聚焦缺陷，禁止批量补勾。
- **验收**：
  - [x] 15 张基线卡逐项记录判定与证据；DONE 卡不存在无承接的必要验收缺口
...
- **ISS-075 对账结论**（2026-09-19 深夜，worker ctx_7c82090d1808，分支 `iss-075-evidence-reconcile` 基于 `9cf401e3531b394e22791b6c5e0177355dbaff13`；`origin/main` 已前进至 `5205be8609120f2c814539171d1cb611d6ffe7b8`（#120，差异仅本卡索引行与本证据行的派发登记），本工作区 shell 精确白名单不含 merge/rebase 无法就地收敛，本次提交与 #120 无行级重叠，由 PM 合并时自动收敛）：
  ...
  - **基线 15 卡逐项判定**（分类：A=已存在可复查证据，本次逐框补勾并就地附引用；B=已明确迁移给父卡，维持未勾；C=仍未验证，维持未勾。33 框补勾均逐框附证据，非批量）：
    - ISS-002A（#97 → `7a869658c2aa0bbf5e6771a88bf53fb2bb605bc5`）：4 框全 A 补勾；REJECT 接缝的处置见下「组合合同」。
    - ISS-059（#71 → `d53a7af64ff86586f61e3363a42695cfc673e1b2`）：1 框 A 补勾——原 `NOT_VERIFIED` 的握手页实机渲染已由 #110 场景 B 补齐，机器可验部分证据原本在册。
    - ISS-068（#100 → `f8c73891cb5aae370b544c008b97fa5965c7c990`）：3 框原已勾；余 1 框 B 维持未勾（真机 invoke/TCC 面板留 ISS-002 人工门，不冒充已过）。
    - ISS-069（#103 → `81578073071ed6d1aef85352f5ca9348eefdd354`）：4 框全 A 补勾（PM 代跑 86/86、浏览器 39、零后端改动、口径与负向文案断言均在册）；实机 du -I 生效留 ISS-009/ISS-010（卡内原登记，非本四框内容）。
    - ISS-040A（#104 → `95dd3edd4e0e90b528deb149907920196159da69`）：4 框全 A 补勾（24 测试函数在册、schema 逐字段、幂等/552 四处同步、仅标准库）；独立 reviewer 缺席属流程例外（卡内已登记原因），非验收缺口。
    - ISS-070（#107 → `4e16a3449e73ea146878cc54634f313b28653fdf`，docs #108 → `43bb49d9abb0b5df21c7b72d7e037ef031054ea6`）：**已交付**=诊断进度字段（框 2 A 补勾）与门禁不回退（框 4 A 补勾）；**未定位**=run 5 根因（框 1 维持 C——对照实验 A/B/C 只支持「本实验未复现填充率影响」，不构成证伪；框 3 条件未触发维持未勾）。诊断交付与根因未定位分开陈述，有限复测不写成因果证明；4.1 倍/24 倍与终点累计数两点更正已在卡首更正块（#119）。
    - ISS-061（#77 → `67892454a01f9452a87039db8b304a0b6abfde8e`）：1 框 A 补勾——生产规模完成并写入快照已由 ISS-001 终态证据（run 4/run 6 done，快照 #2/#3）兑现。
    - ISS-054（链随 #61 → `a158889d980e8131919f44dcb1a0d58b86516c30`；本卡 head `c52cdefe72735d41264b71f4ae9e526078f4cfe9`）：3 框全 A 补勾（PM 复跑 cargo check exit 0；diff 仅类型/可变性；占位图标口径留档且已被 ISS-045 正式图标取代）。
    - ISS-056（链随 #61）：3 框全 A 补勾——前提被 2026-09-15 更正推翻后，目标（各段真实执行+确定结论）由 053/055/057 链达成（verify 12/12→22/22）。
  ...
  - **下一 PM 可直接复核引用清单**（PR → squash 合并 main（40 位）；运行记录在各卡证据段，无需安装任何旧编排工具）：
    | PR | 内容 | 合并 main |
    |---|---|---|
    | #10 | ISS-026 R3 原型 | `0faeda686625f59ef3e9bccb246650bf02c02978` |
    | #45 | ISS-029 切片 2 | `87d452a165cda4ff6bd6cf1a360677aaa5fc3d5f` |
    | #49 | ISS-028 实装 | `f08b93523f5b7075ce8098620d1226bd656f0fc5` |
    | #57 | ISS-037 版本/来源 | `b27404aee20b568deb494344c85a5cd1cb5140d0` |
    | #61 | ISS-009 切片 1（含 053/054/055/057 链） | `a158889d980e8131919f44dcb1a0d58b86516c30` |
    | #71 | ISS-059 | `d53a7af64ff86586f61e3363a42695cfc673e1b2` |
    | #77 | ISS-061 | `67892454a01f9452a87039db8b304a0b6abfde8e` |
    | #79 | ISS-062（verify 20/20 基线） | `47391ae35d5122a931e832e9889a05000936b821` |
    | #97 | ISS-002A（REJECT 接缝源） | `7a869658c2aa0bbf5e6771a88bf53fb2bb605bc5` |
    | #99 | ISS-067 API 补列 | `3c145a5151efc954f1779aad4c99c7d394b581ba` |
    | #100 | ISS-068 opener | `f8c73891cb5aae370b544c008b97fa5965c7c990` |
    | #103 | ISS-069 编辑器 | `81578073071ed6d1aef85352f5ca9348eefdd354` |
    | #104 | ISS-040A 清单工具 | `95dd3edd4e0e90b528deb149907920196159da69` |
    | #107 | ISS-070 进度线索 | `4e16a3449e73ea146878cc54634f313b28653fdf` |
    | #108 | ISS-070 转 DONE 文档 | `43bb49d9abb0b5df21c7b72d7e037ef031054ea6` |
    | #110 | ISS-009 切片 2 实机 | `f349b780bac0cb266e7d7e1f8dbfc086ec62090a` |
    | #112 | DMG 放行提示（DEC-022） | `c307040d4e3d0d7c4309552851d2caff991c254e` |
    | #114 | ISS-072 视觉实装 | `a568b1d8f3a7a7e2389296806fe448c79f642bbc` |
    | #116 | ISS-045 深度环图标 | `408722a86adf58a360538988ca34404f0c6f8694` |
    | #117 | ISS-045 深潭修订 | `55d6c3fe5072311fdc2693866136ba5e9e6409b1` |
    | #118 | ISS-073（前端 88 基线） | `a9c736745e296c3b2da7ddf49a34ad581eb8ebd5` |
    | #119 | ISS-074 审查（对账输入） | `9cf401e3531b394e22791b6c5e0177355dbaff13` |
  - **独立 reviewer 复核（2026-09-19 23:2x，对抗性审阅 head `63383fc`）**：**ACCEPT**，未发现 blocking。reviewer 自查算术（33 补勾 = 01A,02,04A,07,09,26,28,29,37,54,56,59,69,70,02A,61,57）+（−2 移出 057/068）+（保留 08,68）= 框数差，与索引 −8 净额吻合；逐 SHA 40 位校验；独立复跑四入口（Python 3.14.6、vendored echarts fp 匹配、83/83+5skip 或 88/88、脚本语法检查）；核实 ISS-067 无独立 reviewer 记录之说属实（#99 无 review 无评论）。
  - **PM 处置 reviewer 三条非阻断观察**：①**O-1 SHA 笔误已修**——#116 的 merge SHA 第 31 位 `c`→`f`（原值不可解析，违背「引用可复查」；PM 用 `git cat-file -e` 独立验证）；②**O-2 措辞精确化**——ISS-037 补勾理由原引「卡内明示」，实为**卡内证据段**记录（该措辞最早出自 #58），已改为「卡内证据段记录…本次据该记录与实地核验补齐」；③**O-3 卡头不一致已在卡内统一**——ISS-053/054/056 卡头 `READY`→`DONE`（索引为权威），注意 057/068 属 reviewer 核实的「合理未勾」未动。
  - **PM 运行级复核（worker 记 NOT_RUN 的三入口，PM 代跑）**：①`pytest tests/test_api_config.py` **46 passed**；②`node scripts/verify_frontend_refresh.cjs` **88/88**；③`bash scripts/ci_tauri_opener_registered.sh` **exit 0**（源码注册 + ACL opener/allow-open-url + schema scope）——③需先 `rustup run 1.88.0 cargo build --locked --offline`。三项补齐后，本卡 4 项验收全部达成。

### ISS-076 · 剩余发行实测准备与环境缺口清单

> **当前使用方式（2026-09-22）**：本卡以下证据是 09-19 准备工作的历史验收，不是当前实现判断或派发顺序。010B/016B/040B/078 后续已有交付；现状见 ARCHITECTURE 与已刷新实测清单，当前接续只见首页摘要。原“ISS-010B 应用内更新接线”是标题误记，010B 实为后台注册桥。
...
- **状态**：WAITING（P2/M1；2026-09-22 对账：代码已由 #130 合为 `18b5216`，框 2 仍待前台实测；不是等待再次合并）；来源：ISS-028 三尺寸验证的结构层发现（2026-09-20，[PR #125](https://github.com/cat-xierluo/fathom/pull/125) 证据段登记）。
...
- **验收**：
  ...
  - [x] 反例：端口范围全部被占 → 握手页显示 ports-exhausted 与恢复指引，无未知进程被发信号 —— 机器可验部分全 pass（verify `i-no-fathom-health` / `i-zero-kill`（7953..7956 dummy pid 前后一致）/ `i-shell-alive` / `i-ports-exhausted-marker` / `i-exit-cleanup`；单测 `ports_exhausted_status_json_shape` 等 4 项覆盖 `state=exhausted`+`recovery`，reviewer 逐行确认前端字段名一致）；**握手页实机渲染已由 ISS-009 切片 2 场景 B 实机补齐**（PR #110 → main `f349b780bac0cb266e7d7e1f8dbfc086ec62090a`，2026-09-18：dummy 占满 7960..7964，握手页实机截图「握手未完成」+ `state=exhausted` + 端口候选 7960–7964 + 恢复指引，dummy PID 前后一致零击杀；ISS-075 对账补勾，判定 A——原 `NOT_VERIFIED` 项消除），并入 ISS-009 验收框"后台未就绪可恢复"
...
- **验收**：
  ...
  - [ ] 真机或最小集成证据证明 invoke 成功 —— **NOT_VERIFIED**：本环境无 `.app` 产物（`verify_app_bundle.sh` 报 `BLOCKED：未找到 .app`），且 macOS TCC 完全磁盘访问面板跳转需实机确认；不以 mock 结果冒充；ISS-075 对账：维持未勾（判定 B）——原生 TCC 面板跳转留 ISS-002 人工门（本卡证据段原登记），静态链（lib.rs:366 注册、capabilities scope、gen/schemas 产物、门禁脚本在册）已由 ISS-075 独立复核，见 ISS-075 证据段
...
- **验收**：
  - [x] 新增/删除/非法拒绝/确认提示各有前端检查项（65→65+N 全过，PM 代跑）—— ISS-075 对账补勾（判定 A）：10 项检查实际 76→86，PM 代跑 86/86（PR #103 → main `81578073071ed6d1aef85352f5ca9348eefdd354`）；检查项名单与两轮修复 episode 红绿记录见本卡证据段
  - [x] 保存走既有 PUT 入口（fixture 计数），失败时旧值保持且可辨 —— ISS-075 对账补勾（判定 A）：`saves-via-put-and-hints-reinstall` 计数断言 + `env-pinned-never-puts` 第二道防线；B1（hint 回显）/B2（确认勾选复位）修复有红绿对照（81 passed/2 failed → 83/0，卡内）
  - [x] 修改提示与数据集身份口径一致；无「数量=影响」类表述 —— ISS-075 对账补勾（判定 A）：确认提示「保存后下一次扫描将形成新数据集，不与旧数据互比」与 ISS-021/066 口径一致；`frontend/modules/pages/settings.js:486` 注释与检查脚本负向断言钉住无「数量=影响」表述
  - [x] 浏览器 39 项不回退；不改后端 —— ISS-075 对账补勾（判定 A）：PM 独立复跑浏览器 39；`git diff --name-only` 边界确认仅 frontend/ + 夹具脚本 + 卡片、零后端改动（卡内证据段）
...
- **验收**：
  - [x] fail-closed 矩阵全部有测试（缺失平台/版本不一致/空 sig/坏 JSON）—— ISS-075 对账补勾（判定 A）：`tests/test_update_manifest.py` 24 个测试函数在册（本对账 grep 复核）；fail-closed 8 例 rc=1 且不落盘、校验器对 6 类篡改清单拒绝、三处守卫负向对照各有绿转红记录（卡内两轮实测）
  - [x] 生成的 latest.json 字段与 Tauri v2 updater schema 一致（对照官方文档字段名，写进 RESULT）—— ISS-075 对账补勾（判定 A）：schema 依据官方文档逐字段（依据与落地位置见卡内证据段）；注：本卡无独立 reviewer 属流程例外（两次派发超时、PM 直接审阅并独立复跑，卡内已如实登记），非验收缺口
  - [x] 幂等；`python3 -m py_compile`/全量 pytest 计数同步 —— ISS-075 对账补勾（判定 A）：同 `--pub-date` 两次运行 byte-identical、失败不破坏既有清单；py_compile 两脚本 ok；全量 552 由 PM 复跑并四处计数同步（卡内）
  - [x] 不引入网络请求/签名实现 —— ISS-075 对账补勾（判定 A）：两脚本仅标准库、0755（PM 核对，卡内）；`.sig` 由调用方提供，无签名实现
...
> 2026-09-19 审查更正：以下保留原调查记录。受控小卷与事后复测只表明本实验条件下未复现，不足以“证伪”生产窗口的磁盘因素；系统竞争仍是线索，run 5 根因未确认。14400 秒相对于 59 分钟约 **4.1 倍**，不是 24 倍。一次超时累计条数不能单独区分持续缓慢与中途卡住。本卡 DONE 仅描述已合并的进度诊断修复，原验收缺口由 ISS-075 对账，不自动补勾。
...
- **验收**：
  - [ ] 给出磁盘压力与 du 耗时的**对照实验证据**（至少一组：当前 99% 占用 vs 系统正常占用时的同一子集实测），明确**确认或证伪**"磁盘接近满载拖慢 du"这一假设 —— ISS-075 对账：**仍未验证**（判定 C）。对照实验 A/B/C 已执行并留档（本卡 2026-09-18 段），但按 2026-09-19 审查更正只支持「本实验未复现填充率影响」，不构成对生产窗口磁盘因素的证伪；run 5 根因未定位，按 ISS-001 观察窗口与审查原则跟踪（再中断则原卡补匿名证据再判定），不据此改时限或开重写卡
  - [x] 超时路径的 message 补充可诊断字段（先红后绿：构造超时用例断言新字段存在且可读）—— ISS-075 对账补勾（判定 A）：`test_timeout_message_reports_partial_progress_count` 先红后绿（卡内）；`fathom/scanner.py:323/378-379`（`_count_du_records`/「已产出 N 条记录」，本对账 grep 固定位置）；PR #107 → main `4e16a3449e73ea146878cc54634f313b28653fdf`，pytest 552→553
  - [ ] 若确认磁盘相关：前端/日报能在 `interrupted` 时呈现**可解释原因**（沿用 ISS-002A/065 的可解释文案口径，不出现「数量=影响」表述）—— ISS-075 对账：条件项未触发，维持未勾——磁盘相关未获确认（第 1 项），不得以第 2 项的进度线索交付冒充本项；若后续确认磁盘相关再回本项
  - [x] 全量门禁不回退（当前 main：pytest 552、前端 86、浏览器 39、cargo test 25）—— ISS-075 对账补勾（判定 A）：pytest 553（#107 合并即同步，#118 复跑仍 553）；前端 86→88（#118 reviewer 独立复跑 88/88）；浏览器 39（#118）；cargo test 25——`f8c7389` 起 `apps/desktop/src-tauri` Rust 源与 Cargo.toml/Cargo.lock 零改动（`git log` 核实，ISS-075 对账），结果结构性保持
...
- **验收**：
  - [x] 三态覆盖文案 + vanished 缺失防御在 `verify_frontend_refresh.cjs` 有对应检查（含浏览器降级路径不渲染假链接）—— ISS-075 对账补勾（判定 A）：7 项 `coverage-*` 检查 + 浏览器降级 `permissions-browser-fallback-shows-path-text`（`scripts/verify_frontend_refresh.cjs:1621-1731`，降级路径只显路径文字不渲染假按钮）；PM 复跑 76/76（PR #97 → main `7a869658c2aa0bbf5e6771a88bf53fb2bb605bc5`）；现役套件 88/88（#118 reviewer 复跑）
  - [x] 深链按钮在 mock Tauri 桥下被检查脚本驱动并验证；图标只经 icons.js —— ISS-075 对账补勾（判定 A）：`permissions-tauri-mock-deeplink-invokes-opener`（cjs:1793）；图标经 icons.js（卡内边界确认；ISS-073 再证新增环图形亦从 icons.js 导入，无散落副本）
  - [x] 前端检查计数同步（PM 运行 node 命令并同步）；浏览器 39 项不回退 —— ISS-075 对账补勾（判定 A）：第三 episode 后 PM 独立复跑 76/76、浏览器 39/39、pytest 524、cargo test 23（卡内 PM 独立复跑段）
  - [x] 文案不出现"数量=影响"或"未记录=已删除"表述 —— ISS-075 对账补勾（判定 A）：机检负向断言在册（cjs:1632-1633：coverage 文案不含「数量=影响」「未记录=删除」）；`overview.js:53`、`settings.js:486` 同口径注释
...
- **状态**：DONE（P1/M2，2026-09-20 14:5x；[PR #131](https://github.com/cat-xierluo/fathom/pull/131) squash 合并为 main `2162e4a`，repair1 后 reviewer ACCEPT 0 blocking；合并后 main 四门禁绿：pytest 578/浏览器 39/品牌几何/cargo locked）；来源：ISS-076 清单 §6 推荐（2026-09-20 PM 审阅后登记，[PR #124](https://github.com/cat-xierluo/fathom/pull/124) 合同要点 + `docs/plans/2026-09-19-release-live-verification-prep.md` §6 四条理由）。关键路径最前：ISS-016/ISS-040 → ISS-041 → ISS-030/033 均排在其后。
...
- **验收**：
  - [x] 反例先行：按新合同改写 grep 守护测试后先红（证明现有探针确实拦新写路径），实现后绿；fake 环境注册/注销/状态一致/失败回滚全覆盖（Rust+Python）
  - [x] 用户同意流：开关开启前有解释与确认步骤；取消/失败回落原状态；关闭开关走 unregister 且状态一致
  - [x] `bash scripts/ci_cargo_locked.sh` 全绿（cargo test 计数只增不减）；pytest 全绿且计数同步 `scripts/ci_pytest.sh`/`ci.yml`/TESTING/ARCHITECTURE —— PM 于 72c290f 完成计数调和（553+18(010B)+7(077)=**578**、cargo **42**）并实跑：ci_pytest 578=578、ci_cargo_locked ok、cargo test 42 passed、合并后 main 四门禁绿；reviewer 复核算术一致
  - [x] grep 守护：真实写路径仅存在于注册命令模块；`dry_run_plan` 纯展示不变量有测试；零真实注册发生（本机 `launchctl print` 对照前后一致可查）
- **证据/接续**（2026-09-20 worker 交付，PM 复核待办）：worker task_83235eed08aa（glm-5.3，分支 `iss-010b-register`，base `eb50357`——push 前 rebase 对齐 origin/main #128/#129 两个 docs commit，无冲突）交付 4 commit：`2aa64b2`（反例先行：守护升级 + fake 用例，先红）→ `bf1f20c`（launchd.py 发行态写路径）→ `cbbab4b`（autostart.rs 注册命令模块 + lib.rs 接线 + Rust 单测 16 例）→ `fc81c54`（设置页开关）。**先红后绿两轮实录**：改写探针后 `.runtime/bin/python -m pytest tests/test_launchd_autostart.py -q` 退出码 1（**18 failed / 23 passed**，新探针全红——AttributeError 缺 register_release/release_plan、autostart.rs 缺注册命令与写动词、lib.rs 未接线；红证据存 Session Context `evidence-red-run1.txt`）；实现后同命令退出码 0（**40 passed**，22→40）。**验证命令**：`bash scripts/ci_cargo_locked.sh` 退出码 0（仅 3 条既有 dead_code warning，先于本切片）。**fake 覆盖清单**（Python 18 例新增 + Rust 16 例）：拒绝未确认（零命令零文件）、uid 失败 fail-closed、成功注册（plist 内容 helper argv + scan 时间注入 + 命令序列 bootout/bootstrap×2）、bootstrap 失败 rc=5 两标签回滚（4 次 bootout + 双 plist 删除）、spawn 失败回滚、目录/第二份写失败零 bootstrap 且清理已写、注销 bootout 按 label + unlink、幂等（plist 缺失仍 ok）、Could not find service 不计 warning、unlink 失败 ok=false 指名标签；release_plan/dry_run_plan 纯展示（monkeypatch 零执行）。**零真实注册**：worker 全程仅执行 pytest 目标用例与 cargo build 门禁（本会话命令清单可查），无任何 launchctl 写命令、未触碰 `~/Library/LaunchAgents`。**计数**：pytest 553→**571**（+18，仅 tests/test_launchd_autostart.py），`scripts/ci_pytest.sh` 已同步；全量 `pytest tests -q` 与 `cargo test` 的执行被 worker shell 守卫拒绝（不在 allowed_shell_commands：仅两条合同命令 + safe-push + git 只读），**571 为算术推导（553+18，其余测试文件未动，launchd.py 重构保 dev 输出逐字节不变由既有同源测试钉住），NOT_RUN-需PM 复跑全量确认**；cargo test 计数 **NOT_VERIFIED-需PM**（build 已过、单测已写）。**PM 收口清单**：① 复跑 `.runtime/bin/python -m pytest tests -q`（预期 571）与 `rustup run 1.88.0 cargo test --locked --offline --manifest-path apps/desktop/src-tauri/Cargo.toml`（预期 25+16=41 只增不减）；② 计数同步 `ci.yml:43`、`docs/TESTING.md:14,44`、`docs/ARCHITECTURE.md:100`（553→571；三文件不在 worker 写入范围，scope guard 阻断）；③ **Tauri ACL 授权缺口**——✅ 已闭环（2026-09-20 21:4x，TASK-ISS-010B-ACL worker，分支 `iss-010b-acl` base main `03f7bc3`，[PR #133](https://github.com/cat-xierluo/fathom/pull/133)，交付 head `e39281f`+收口 commit，合同 `.git/orchestration/wave35-evidence/ISS-010B-ACL.prompt.md`，Session Context RESULT/PATCH_SUMMARY/PR_BODY 在 worktree `.claude/agent-sessions/iss-010b-acl/`）：新增 `permissions/autostart.toml` 四条权限（identifier `allow-autostart-status/-register-plan/-register/-unregister`，commands.allow 与命令一一对应，中文 description 含 confirmed 同意语义）+ `capabilities/default.json` 于 allow-update-tray-status 后追加四条（remote urls 未扩域、未加其他权限）+ fail-closed 守护 `tests/test_tauri_autostart_acl.py` 8 例（①capability 授权齐 ②TOML↔lib.rs generate_handler 命令一一对应（删命令/改名/漏权限必红）③capability 里 autostart 权限恰四条防顺手放宽 ④update-tray-status 与 core/opener 基线及 remote urls 不回退）。先红后绿：缺口态 6 failed/2 passed → 修复后 8 passed；漂移反例（TOML 命令改名 `autostart_disable`）3 failed/5 passed → 还原后 8 passed。双验证：`.runtime/bin/python -m pytest tests/test_tauri_autostart_acl.py -q` 退出 0（8 passed）、`bash scripts/ci_cargo_locked.sh` 退出 0（tauri-build 编译期校验 capability 引用通过；仅 3 条既有 dead_code warning，先于本切片）。计数 578→**586**（+8）同步 `scripts/ci_pytest.sh`/`.github/workflows/ci.yml`/`docs/TESTING.md`/`docs/ARCHITECTURE.md` 四处（cargo 42 未动）；全量 `bash scripts/ci_pytest.sh` 被 worker shell 守卫拒绝（不在 allowed_shell_commands），586=578+8 算术推导（其余测试文件未动），NOT_RUN-需PM 复跑。原缺口背景（ISS-010A 起autostart_status 就未授权；未授权时设置页 invoke 会被 ACL 静默拒绝，ISS-068 同型缺口）；④ 浏览器/前端全量检查（mock 桥对新面板返回 undefined 已做形状校验降级，预期不影响既有 65 项）；⑤ 发行 plist 不含 EnvironmentVariables 钉 FATHOM_RUNTIME_DIR（与 dev 态同形；非默认运行根下 launchd 拉起的 helper 会回落默认运行根，G8 实机若见数据分叉在此补）。设置页同意流已按合同实现（计划展示→确认→执行→状态回读；取消/失败回落由系统状态回写开关），浏览器级断言留④。关联：父卡 ISS-010、ISS-010A（只读桥与夹具先例）、ISS-076 清单 §4.1/§6、G8 人工门。
...
- **验收**：
  ...
  - [x] 真实生产规模下扫描能完成并写入快照 —— ISS-075 对账补勾（判定 A）：ISS-001 终态证据（2026-09-19，本仓在册）——run 4（09-17，done，62.5 分钟，快照 #2）与 run 6（09-19，done，44.9 分钟，快照 #3）两次真实生产规模定时扫描完成并写入快照（launchd 定时触发，只读观察记录于 ISS-001 证据段）；原 `NOT_VERIFIED` 前提（合并后首次定时扫描尚未发生）已消除，原记录：合并后首次定时扫描为 2026-09-16 12:00（launchd `com.maoscripts.fathom-scan` 以仓库 `.venv/bin/python main.py scan` 运行，生产目录 main 已含本修复），届时只读观察 `scan_runs`/`snapshots`；"或按合同明确记录为未完成且可恢复"一半已由测试与代码路径满足（超时→`interrupted`+message+保留旧快照）。注意常驻 `fathom-web`（PID 6026）仍是合并前进程，经 API 触发的扫描在其重启前仍用 3600s；重启属生产操作，须用户授权
...
- **状态**：DONE（P0/M2，2026-09-15；索引已记 DONE，本次对账统一卡头）；来源：PM 在 ISS-053 修复后复跑 cargo check 发现（2026-09-15）。
...
- **验收**：
  - [x] 干净克隆上 `cargo check --locked --offline` exit 0 —— ISS-075 对账补勾（判定 A）：PM 在 head `c52cdefe72735d41264b71f4ae9e526078f4cfe9` 独立复跑 exit 0（仅 2 个既有 dead_code 警告，卡内）；ISS-055 证据段另有 PM 与 reviewer 在无 helper 产物新 worktree 各自复证；随 PR #61 合并
  - [x] 修复仅涉及类型/可变性/图标产物，helper 生命周期语义与 tauri bundle 其它字段未被放松 —— ISS-075 对账补勾（判定 A）：`c52cdef` 差异仅 `helper.rs`/`lib.rs` 两文件 10+/7-（本对账 `git show --stat` 复核：`u16::from` 两处、`Url` 解析、`guard` 加 mut），无生命周期/打包字段放松
  - [x] 图标若为占位：RESULT 明确标注 NOT_VERIFIED 且指向 ISS-045 —— ISS-075 对账补勾（判定 A）：占位图标由 ISS-053 修复链加入（`0856bf3ba776e5d1699ce50c813515731de65c27`，经 PR #61 入主干），占位口径与「公开前必须换 ISS-045 正式图标」在 ISS-009 切片 1 决策留档；ISS-045 已交付正式深潭图标（#116 → `408722a`、#117 → `55d6c3f`），18pt 小尺寸留 ISS-009 人工门
...
- **状态**：DONE（P1/M2，2026-09-15；索引已记 DONE，本次对账统一卡头）；来源：PM 在 ISS-055 后复跑 verify 发现（2026-09-15）。
...
- **验收**：
  - [x] 在“7952 被外部进程占用”的当前环境下，(c)(d)(f) 能真实执行并给出确定结论（pass 或带证据的 fail），不再是启动链路导致的不确定失败 —— ISS-075 对账补勾（判定 A）：本卡前提已被 2026-09-15 更正推翻（启动链路正常，旧败因是 bundle 内 helper 为目录）；更正后 (c)(d) 全 pass、f 失败为真实壳缺陷转 ISS-057 修复；整链 verify 12/12（#61）→ 20/20（#71）→ 21/21（#110）→ 22/22（#112/#117），各段真实执行到末尾
  - [x] 不放松任何既有断言（a/b/g 与结构/只读/零击杀语义保持）—— ISS-075 对账补勾（判定 A）：演进中 record 名与断言一字未改（ISS-059 卡内确认；段数 12→20→21→22 只增不减）
  - [x] 失败时有可读原因（哪个环节、什么证据），便于定位 —— ISS-075 对账补勾（判定 A）：f-port-closed 失败带可读原因并据以定位为 ISS-057（PM 记录在卡）；unbound variable 提前退出由 ISS-055 修复后全段跑通（本卡 2026-09-15 更正段）
...
- **验收**：
  ...
  - [ ] tray 退出路径仍正常回收且退出码 0 —— ISS-075 对账：维持未勾（判定 B，已明确迁移）——tray 菜单真实点击留 ISS-009 验收框 2（开放，NOT_VERIFIED 清单在册）；非 tray 退出路径已由 ISS-009 切片 2 场景 A 实机复证（PR #110：AppleScript quit 后 helper 回收/端口关闭/instance 清理）；原记录：tray 菜单真实点击属 GUI 交互，本卡 `NOT_VERIFIED`；代码层 tray 路径 `quit_with_helper` 与全局钩子汇合到同一 `reap_spawned_helper`（reviewer 逐行 CONFIRMED，lib.rs:275-291/370-374），实机点击并入 ISS-009 验收框"关闭/退出行为正确"
...
- **状态**：DONE（P0/M2，2026-09-15；索引已记 DONE，本次对账统一卡头）；来源：ISS-009 切片 1 PM 复跑发现（2026-09-15）。
...
- **验收**：
  - [x] 干净克隆（无 resources/helper/fathom-helper）上 `cargo check --locked --offline` exit 0 —— ISS-075 对账补勾（判定 A）：glob 硬错误已消失（`0856bf3` 改 `**/*`，本卡 01:20 进展段）；PM 与 reviewer 在无 helper 产物的新 worktree 各自实证 exit 0（ISS-055 证据段）；随 PR #61 合并（main `a158889d980e8131919f44dcb1a0d58b86516c30`）
  - [x] 跑过 build_helper.sh 后 `tauri build` 仍把 helper 树打进 .app 的 Contents/Resources/helper/ —— ISS-075 对账补勾（判定 A）：ISS-055 深键映射 `{"resources/helper/fathom-helper/": "helper/"}` 落位 `Contents/Resources/helper/fathom-helper/fathom-helper` 且 `file` 实证 Mach-O arm64（卡内）；verify (b) 段只读布局指纹一致
  - [x] 反例：构建脚本不再因 glob 无匹配而中断；.gitignore 仍排除产物 —— ISS-075 对账补勾（判定 A）：红态（glob 无匹配阻断 cargo check）复现于本卡范围段与 PM 复跑记录；修复后无产物可 check；README.txt 占位 + .gitignore 取反保证产物零入库（ISS-055 `b08a5a1`，卡内）
...
- **实施边界**：这是与 PR #10/ISS-026 原型视觉确认不同的第二个人工门。仓库外现有 A/B/C 概念板仅供选择；PM 推荐“A 的深度环骨架＋B 的一层轻微不规则等深线”。用户尚未确认，选择前不把概念稿或派生资产写入仓库，也不把它当作发行资产。（该门已于 2026-09-19 由用户确认关闭：深度环为正式方向，见验收第 1 项与证据段。）

**层叠深潭形态交付（2026-09-19 下午，双形态体系）**：

- **后续选择回写（Logo 设计会话）**：用户确认“层叠深潭”第二轮图案，明确要求只取核心作为 Logo。此形态不同于此前 PR #116 的线条深度环；已勾验收项仅对应旧形态，不能视为新图验收。所选原稿与提示词存 `assets/brand/`（入库）。内置 image_gen 透明提取未过 alpha 检查，最终由 PM 以本地 Pillow 像素抠图落地。
...
- **验收**：
  ...
  - [x] 生成 canonical SVG，以及 1024/512/256/128/32/16 PNG、完整 iconset/icns；生成脚本可重复运行并核对像素与 alpha——canonical `apps/desktop/src-tauri/icons/icon.svg`（24 viewBox 与 brandRing 逐参数对应）；`scripts/build_app_icon.py` 渲染 1024 源（`--force` 可重复）；`scripts/build_icons.sh` 生成全尺寸 iconset + icns + tauri 单文件，sips 核对 16/32/128/256/512/1024/icns(512)/tray(44) 像素与 hasAlpha 全部通过
...
- **验收**：
  ...
  - [x] 用户评审针对具体产物，反馈回写 DESIGN；不能只产出一张不可操作的静态美图 —— ISS-075 对账补勾（判定 A）：用户 2026-09-14 对可交互 R3 原型（114/114 原型检查、13 截图）确认合并（「先合并吧，后续有问题的时候我会再给你提意见」，记录在本卡证据段）；原型第三轮合同在 DESIGN（docs/DESIGN.md「原型第三轮」节）；2026-09-18 用户再审选定 R3 方向 A+B（ISS-072）、09-19 品牌形态转正（DEC-023）——后续反馈持续回写 DESIGN
...
- **三尺寸证据（2026-09-20，worker glm-5.3，分支 `iss-028-tauri-size-evidence` base `f3633c4`；[PR #125](https://github.com/cat-xierluo/fathom/pull/125) 已由独立 reviewer ACCEPT（0 blocking / 3 non-blocking）+ PM 代开后 squash 合并为 main `4181fd28c08fd0668d8c2d3257b4aa6732f7bf29`；合并后 main 门禁 pytest 553 / 浏览器 39 / 品牌几何 OK）**：新增 `scripts/verify_tauri_window_sizes.sh`（fail-closed：0 可执行断言全过 / 1 断言失败 / 3 环境阻塞；.app 缺失时按现有链自举构建——rustup 1.88.0，helper SHA256 `66d5e565…1667` 与 #110 逐字节一致；证据落 `apps/desktop/src-tauri/verify-results/window-sizes/`，gitignore 内不入库）。隔离：EnvProbe.app 先实证启动风格的环境注入（本轮 `open -g --env` 三变量齐传）+ 运行根 settings.json 保险带 + `/api/config` realpath 硬门；合成扫描根（du 实测）两日快照驱动增长/缩减（Media +50MB、Notes −20MB）；生产 7952（pid 6026）前后只读快照一致，真实 HOME 无本时段写入，退出回收/端口关闭/instance 清理全过。命令 `bash scripts/verify_tauri_window_sizes.sh` **退出码 0**（verdict `PASS_WITH_NOT_VERIFIED`；worker 终跑 18/0/14、PM 复跑 18/0/14、reviewer 自举构建实跑 19/0/14——多出 app-bootstrap-build 一项属预期差异；reviewer 运行 `window-sizes/20260919T181829Z/`）：
  ...
  - **结构层发现（不硬修，登记）**：① WKWebView 疑似不向页面送达 Escape keydown——前台模式 Esc×3 未关闭目录详情侧栏（Web/Playwright 证据里 Esc 可用；侧栏「关闭」按钮路径可用），Esc 焦点返回在桌面壳的可用性存疑，建议 PM 拆卡（壳层注入或前端改用可见关闭路径收口）；② Tauri/macOS 桌面自动化零打扰边界（上述通道实证结论）对 ISS-076 的实机验证准备有直接参考意义。

<details>
<summary>2026-09-14 历史交付与派发记录（不得作为当前指令）</summary>
...
- **验收**：
  ...
  - [ ] 最小窗口/标准窗口/大屏及 Tauri/Web 分别有证据；图表有表格替代 —— 2026-09-19 复审撤回 #121 补勾：Web 三视口与图表等价表格在册；2026-09-20 补齐 Tauri 三尺寸几何/总览图表证据与 980×640 前台模式全矩阵（见上「三尺寸证据」段），但溢出/键盘/详情在 1220/1440 及零打扰模式下仍 NOT_VERIFIED-需前台（14 项），三项子证据未在三个尺寸齐备——按合同维持未勾，状态按索引为 WAITING，待前台窗口复验后收口。
- **证据/接续**：三尺寸验证脚本与两轮实机证据见上「三尺寸证据」段（合并于 #125，main `4181fd2`）；后续：14 项需前台断言按 2026-09-20 用户规则由 PM 在用户离线窗口亲自复跑（同一脚本约 5 分钟）后勾选第 4 框并转 REVIEW；Esc 送达问题已拆卡 **ISS-077**（代码已合并、前台验证等待中，以索引为准）。
...
- **验收**：
  - [ ] 新账户运行不依赖开发者 venv/绝对路径，断网能首启 —— ISS-075 对账：维持未勾（判定 B，已明确迁移）——并入 ISS-009 验收框 1（PM 2026-09-14 决策记录在本卡切片 2 段；ISS-009 仍开放，断网首启在其 NOT_VERIFIED 清单）
  - [ ] 后台唯一所有者明确，退出/崩溃/重启/登录语义可实测 —— ISS-075 对账：维持未勾（判定 B，部分证据+迁移）——spike 18/18 合同原型含 8 进程唯一 owner（PR #17 段）；发行级实测（重启/登录语义）按 PM 决策并入 ISS-009 验收框，余项有承接
  - [ ] 安装路径含空格/中文/& 可用；服务无权限/端口占用给恢复动作 —— ISS-075 对账：维持未勾（判定 B/C 混合）——空格/中文/& 路径与端口占用恢复已有证据（verify (c) 段自 #61 起覆盖、22/22 at #117；ports-exhausted 实机 #110 场景 B）；「服务无权限」恢复场景未见专门证据，并入 ISS-009 发行实测清单
  - [ ] 当前宿主架构 helper 的 `file`、断网启动、`--version`/health、退出与崩溃行为有可复查证据；另一架构明确标为待 ISS-041 复验 —— ISS-075 对账：维持未勾（判定 B，部分证据+迁移）——arm64 Mach-O `file`/`--version`/health（G3/G4）与退出（冻结冒烟 21/21）证据在册（PR #45 段）；断网启动并入 ISS-009；x86_64 已标 ISS-041；余项有承接
...
- **历史只读观察补记（从旧接手摘要归位，不是本次复跑）**：
  - **2026-09-20 21:3x 补给（PM 只读复核）**：09-20 定时扫描 run 7 **`done`**（12:00:09→13:46:16，106 分钟，`du_seconds=6192.9s`），快照 3→**4**，日报 `reports/2026-09-20.md` 生成（`report_status: written`、`warnings: []`、快照 #4 与 #3 同数据集 `min_kb=10240`/`partial`、`vanished_count=10`）。**据现有证据观察**：定时链已连续三次成功（run 4 62.5 分钟 → run 6 45 分钟 → run 7 106 分钟），三次 `du_seconds` 为 3531.6 / 2576.2 / 6192.9 秒——**耗时仍在 2.4 倍区间内波动**，与「资源竞争」假说一致，但**样本仅三次、不足以定性**，且未取得竞争进程的直接证据（无 load/进程快照）。**结论口径**：不据此关闭或改写 ISS-070 的根因结论（其交付为「超时进度信息」，该交付已完成且有效）；不把三次成功写成因果证明；**继续按原判据观察**——若再出现 `interrupted`，按 message 的「已产出 N 条记录」判定推进慢/卡死并在原卡补匿名化证据。
  - **2026-09-21 16:3x 补给（PM 只读复核）**：09-21 定时扫描 run 8 **`done`**（12:00:14→12:43:31，43 分钟），快照 4→**5**（snapshot_id=5），日报 `reports/2026-09-21.md` 生成（`report_status: written`、`notification_status: submitted`、`warnings: []`、`pruned: 0`、`source: scheduled`）。**据现有证据观察**：定时链已连续四次成功（run 4 62.5 分钟 → run 6 45 分钟 → run 7 106 分钟 → run 8 43 分钟），**耗时仍在约 2.5 倍区间内波动**（43↔106 分钟），与「资源竞争」假说一致，但**样本仅四次、仍不足以定性**，且本次仍未取得竞争进程的直接证据（无 load/进程快照）。**结论口径**：与上次补给一致——不据此关闭或改写 ISS-070 根因结论、不把成功写成因果证明、继续按原判据观察；若再出现 `interrupted`，按 message 的「已产出 N 条记录」判定推进慢/卡死并在原卡补匿名化证据。附：本次复核同时确认 09-18 run 5 `interrupted`（`du 超过 14400 秒安全时限` + 末段路径）为**唯一**未成功项，其 message 已含安全时限与证据，无需新建卡。
...
- **验收**：
  - [x] 证明来自定时任务而非手动触发，存在两个不同有效日期——快照 #2（2026-09-17T13:02:38，run 4 `done` 62.5 分钟）与 #3（2026-09-19T12:45:04，run 6 `done` 44.9 分钟），两次均由 `com.maoscripts.fathom-scan` 于 12:00 定时触发（launchd 上次退出码归 0，out 日志「开始扫描」时间戳 12:00:07/12:00:08 与 scan_runs.started_at 一致）
  - [x] 日报基于正确同根快照；无变化也能如实表达——首份真实日报 `reports/2026-09-19.md` 头部明示对比对（#2→#3，根与数据集口径），部分覆盖（130 权限/43 消失）如实注记，卷剩余空间较上次变化有呈现；「无变化如实表达」由 reports/reports.py 既有零变化测试覆盖（ISS-042 等），生产首例恰有大量变化不阻碍该判据
  - [x] 耗时/覆盖/库及 WAL 增长的匿名摘要有记录；不得贴私人目录清单——run 6 耗时 44 分 58 秒（du_seconds=2576.2s，快照 #2 为 3531.6s）；覆盖 884,057 目录、130 权限受限、43 扫描期间消失；库 11.37 MB（v5），checkpoint 后无 -wal/-shm 残留；日报 14,080 字节。私人目录清单不入库（日报本身在生产 reports/，按规矩不入 git）
- **证据/接续**：NOT_VERIFIED。2026-09-14 PM 只读观察：`launchctl list` 显示 `com.maoscripts.fathom-scan` 上次退出码 1；`logs/launchd-scan.err.log` 记录 2026-09-13 12:00 定时任务确实触发（out 日志“开始扫描”），但被 `InvalidScanError`（stderr 两行 `Interrupted system call` 判为非权限致命）拒绝，生产库 `data/fathom.db`（v0 schema）仍仅有 2026-09-12 一个快照，第二个有效日期未产生。根因已由 ISS-047 修复并合并，主仓代码已同步；最早 2026-09-15 12:00 后重新观察。原任务“增长非全零”门槛已纠正。
  ...
  **2026-09-19 13:1x PM 终态（本卡 DONE）**：run 4（09-17，`done`）与 run 6（09-19，`done`，12:00:08→12:45:06）提供两个有效定时日期（09-18 的 run 5 中断，并非连续成功），快照 #2/#3 构成同根同数据集对比对，**首份真实对比日报 `reports/2026-09-19.md` 自动生成**（ISS-021 数据集身份口径端到端兑现：快照 #1 为 v3 旧口径 `min_kb=NULL` 正确不可比，#2/#3 均为新口径）。ISS-061/064/065/067 修复链在生产全部实证。另记：观察中发现一个已运行 17.6 小时的孤儿 `yes` 进程（98% CPU，父进程已退、PPID=1，启动时间晚于 run 5 结束、**非 run 5 元凶**），已清除；load 由 48 回落。run 5（09-18 interrupted 263 分钟）的根因仍未确认；系统资源竞争仅为 ISS-070 记录的待验证线索。
...
- **安装界面未签名提示（2026-09-18 晚，用户决策，[PR #112](https://github.com/cat-xierluo/fathom/pull/112) squash 合并为 main `c307040`）**：用户明确签名/公证先不做（Folia 同模式），改为安装界面提示（[DEC-022](DECISIONS.md)）。交付：`scripts/build_dmg_background.py`（Pillow + 系统 Hiragino Sans GB 生成 660x480 背景，产物 `icons/dmg-background.png` 入库，日常构建零依赖；含 reviewer 建议的「脚本比图新」再生成警告护栏）、`tauri.conf.json` `bundle.macOS.dmg`（背景/窗口尺寸/图标坐标；注：Tauri 2 字段为 `applicationFolderPosition` 非 `linkPosition`，`dmg` 段须在 `macOS` 下）、`verify_app_bundle.sh` 新增 (j) 段（DMG 挂载断言背景图 + app + Applications 链接在位；detach 失败路径已兜底）。实测：DMG 挂载 `.background/dmg-background.png` 41KB 在位；Finder 窗口截图目检绿色箭头对准 app→Applications、提示文案完整（未签名说明 + 右键打开/隐私与安全性两步放行）；**verify 全量 22/22 PASS**（第三次复跑确认；前两次各挂 1/4 段均为**运行期外部干扰**——一次是生产 7952 /health 瞬时抖动（404→000，单次 curl max-time 1s 采样），一次是 7953 被来历不明的外部 fathom helper 占用致壳复用（`c-helper-ready` pass 而 instance 不写、`i-exit-cleanup` 把 7953:200 记为外部既有监听为证），结束后均无残留进程；失败段全在只读 (j) 段之前，与本改动无关。(i) 段外部快照的单次采样脆弱性记为观察，暂不修）；ROADMAP M2 与 README 分发说明同步加注。独立 reviewer（agent_ab7a5）5 项审查全 CONFIRMED **ACCEPT**（坐标一致性实查、Tauri schema 字段核对、README 22 项计数实数=23 个 record ID 中 1 个仅异常分支触发），两条 non-blocking 护栏建议已在合并前补齐。
- **切片 2 实机验证与缺陷修复（2026-09-18 晚，PM，[PR #110](https://github.com/cat-xierluo/fathom/pull/110) squash 合并为 main `f349b78`）**：重跑打包链（`build_app.sh` rc=0，helper SHA256 `66d5e565…1667`，独立 reviewer 实算核对）后，以一次性脚本 `apps/desktop/src-tauri/verify-results/slice2-live/live_verify.sh`（gitignore 目录，日志/截图/instance 快照同目录留证）实机验证两场景，10 项断言全 PASS；独立 reviewer（agent_191cd1）5 项审查全 CONFIRMED（含 PyInstaller 6.22.3 `SourceDestAction` 分隔符语义、BSD grep BRE 交替真机复核、bash 3.2 `-n` 检查）**ACCEPT**，其指出的证据哈希笔误（`94e0dc05…`→`66d5e565…`）已修正：
  ...
  - **仍 NOT_VERIFIED（人工门，验收框 1/2 保持未勾）**：无 Python/Rust/Homebrew 新账户从安装（DMG）进入首扫与分布；tray 菜单手点退出（菜单构建有单测，实机手点未做）；断网首启；真实下载产物以开发者机拷贝以外的证据复核；正式深度环图标的 18pt 小菜单栏下 tray 可辨性（Launchpad/Spotlight 检索与深浅色模式已由 ISS-045 于 2026-09-19 GUI 自动化实测通过）；签名/公证（归 ISS-041）。
...
> **2026-09-22 接续**：ISS-010A/010B 与 ACL 修复已合并，不再从只读桥重做。剩余为真实注册/唯一 owner/睡眠与重启合同；非默认运行根的 plist 未传递 `FATHOM_RUNTIME_DIR` 已在 010B 记录，须先明确发行路径与隔离方案，再做 G8。下面“尚未执行”仅指父卡实机验收。
...
> **2026-09-22 接续**：ISS-016A/016B 已交付配置持久化、计划漂移四态和经确认重装；不再声称真实重装入口未实现。剩余为发行账户中设置/运行根/后台计划一致性的 G9 组合验证，原验收框不因子卡通过自动补勾。
...
> **2026-09-22 接续**：ISS-030A 协议夹具已由 #146 合并；仅证明 fake 流程与实际复用的锁/备份模块，不等于应用已具备升级事务。产品入口缺口见 ISS-040；本卡仍承接安装替换后的版本握手、回滚/恢复、卸载/重装与真实 draft 产物验证。不得只补人工签字便关闭本卡。
...
>
> **2026-09-23 合并收口（用户指令合并）**：repair3 已由 [PR #156](https://github.com/cat-xierluo/fathom/pull/156) squash 合并为 `2b70686`（worker head `dae2d83`，patch-id 三重验证）。修正确认：certifi=MPL-2.0 / pluggy=MIT 与本地 METADATA 实测一致（PM 独立复验），另修 idna/pygments/typing-extensions/uvicorn 四处值错（「无已知事实错误」口径达成）；校验器 `audit_python` 补值级交叉核对（负向探针：uvicorn 临时回错值 → exit 1 精确报两侧值 → 恢复 exit 0）；UNKNOWN 按 DEC-024 保留内联说明不阻断。PM 复核：`--audit` exit 0（Python 值级 21/21、SKIP 0、Rust 516/516）、`pytest tests/test_third_party_notices.py` 11 passed、计数 685→696 四处同步、TASKS 未被 worker 触碰。合并例外登记：本 head 无独立 reviewer（交付来自 Wave41 会话 worker 流程，用户明确指令合并；037 缺陷①②③历经 Wave33 三轮独立审查链，缺陷③修复带负向探针证据，PM 按上述复验承担审查责任）。框 2 按修订口径勾选，本卡 DONE；repair2/repair3 分支已清理（repair2 内容经祖先链包含于 #156）。
...
- **验收**：
  ...
  - [x] 依赖与图标等资源来源及必要 notice 齐全 —— 按 DEC-024 修订口径（2026-09-23 用户授权「参照 Folia」）收口为「无已知事实错误」：#156 全表值级核对修正 6 处许可证值错（certifi/pluggy 互换 + idna/pygments/typing-extensions/uvicorn），UNKNOWN 项内联「来源待核」说明不阻断，深潭图标=image_gen 原稿+处理链已在册；`check_third_party_notices.py --audit` 值级核对 21/21 + Rust 516/516 + 章节引用全命中，负向探针 exit 1 反证校验器承重。历史复审撤回记录保留于上。
...
> **2026-09-22 范围澄清**：DONE 仅覆盖下列插件、ACL、状态与确认流合同；不覆盖安装前停写/备份/恢复、进度/取消或真实 N→N+1。剩余生产接线归 ISS-040，恢复与实包验证归 ISS-030；不要从“更新协调切片”标题推断发行协调已完整。
...
> **2026-09-22 静态复审**：ISS-040A/040B 已合并清单工具、插件与最小权限、检查/确认安装/确认重启入口；生产 endpoint 仍为占位。**尚有产品实现缺口，不只是 G10 人工环境**：`src/lib.rs::updater_install` 直接 `download_and_install`，无安装前停写/旧 helper 退出/SQLite 一致备份/失败恢复接线；进度回调为空，安装中进度/取消合同未兑现。`updater_restart` 的退出回收不等于安装前协调。由本卡承接产品接线，ISS-030 承接恢复与实包证据；ISS-030A 只能贡献夹具证据。现有基础握手与 schema 备份工具不证明更新入口已经调用它们。
>
...
- **证据/接续（2026-09-23 PM 合并收口）**：代码已由 [PR #157](https://github.com/cat-xierluo/fathom/pull/157) squash 合并为 `bb6ac2c`（worker head `2aa340d`，patch-id 三重验证）。PM 静态核验：YAML 合法（单 job build-arm64-draft，tag 触发+workflow_dispatch）；**零 `secrets.*` 引用**（上传用 runner 临时 `github.token`）；两处第三方 `uses:` 均固定完整 commit SHA；**无 publish 路径**（全 workflow 无 --draft=false/release edit，收尾断言 isDraft=true）；`createUpdaterArtifacts=false` 覆盖有双重依据（Tauri v2 文档 + 本机 tauri-cli 2.6.2 探针）；`bash -n` 两脚本语法通过；build_app.sh 三处改动（FATHOM_TAURI_BUILD_ARGS 透传默认零行为变化/版本源动态 DMG glob/bash3.2 全角逗号修复）。**状态转 WAITING**：验收框 1–3 需真实 runner（Actions 额度 NOT_RUN，DEC-024）与 tag 首跑，均未执行记 NOT_VERIFIED；额度恢复后由 PM 在 `v*` tag 首跑并回勾。合并例外登记：本 head 无独立 reviewer（用户明确指令合并，PM 静态核验承担）。后续：Wave41 会话已开 `iss-041a-repair1` worktree（2026-09-23 22:1x，基于 a7cc984，疑为构建 venv 相关修复），其交付按新 head 重新验收。
- **证据/接续补充（2026-09-23 Wave41 会话，B-1 登记）**：上段「无独立 reviewer」系收口时点表述；独立 reviewer（fixed head `2aa340d`）于合并完成后返回 **REJECT：1 blocking（B-1）**——release.yml 构建 .app+DMG 步骤经 `build_app.sh` 内部**无参**调用 `build_helper.sh`，退回不入库的本机缺省 venv `apps/desktop/experiments/iss029/.venv-build`，干净 runner 上必 BLOCKED exit 3，验收框 1 在修复前无法达成（worker 本地全绿正因本机存在该 venv）。上段「疑为构建 venv 相关修复」的 repair1 即 B-1 修复：`FATHOM_BUILD_VENV` 透传（unset/空串时与旧版逐字一致，本地工作流零变化）+ `--target "$RELEASE_TAG"` 歧义修正（原 review NB-2），由 [PR #160](https://github.com/cat-xierluo/fathom/pull/160) squash 合并为 `2195e0d`（worker head `f2fd85d`，真实变更恰 2 工程文件 +21/−2；其分支基线 `a7cc984` 早于 #159，两点 diff 的文档回退为假象，以 merge-base 与 GitHub files 视图为准）。修复轮独立 reviewer（fixed head `f2fd85d`）**ACCEPT 0 blocking**：bash 3.2.57 实测 PIPESTATUS 捕获/if-else 三态/含空格路径引号安全，`FATHOM_BUILD_VENV=/nonexistent` 透传 trace 与反例 exit 3 复现，真实冻结段 exit 0（49s），全量 697 passed。NB 留档（后续优化，非阻断）：NB-1 `HELPER_RC` FAIL 分支为 `a7cc984` 基线既有 dead code（errexit 先于赋值终止；失败传播不受影响，CI 成功路径读取正常）；NB-2 CI 上 helper 冻结跑两次（前置步骤+build_app 内部各一次，第二次 rm 后重冻 ~49s）。合并后 main `2195e0d` 全量 **697 passed** + 版本一致。041A 维持 WAITING：代码侧无已知阻断，等 Actions 额度/公开路径决策 + 用户定 tag 后首跑回勾验收框 1–4。
