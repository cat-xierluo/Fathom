...
- **应用内更新**（v0.3.1 起）：检查更新 → 确认下载安装 → 确认重启；安装前自动停写与备份数据库，升级期间持续停写并拒绝重入；安装失败可自动恢复旧版与数据（见已知限制）。
...
- 应用内更新：升级前自动落盘旧版与数据库恢复材料，安装失败可自动恢复并可接续；真实安装环境的端到端验证仍在进行，遇到更新异常请保留安装日志反馈。
