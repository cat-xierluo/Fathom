...
仓库的三个 fail-closed 入口会建立/核对各自环境和精确通过数；本次审查基线（ISS-077 后）的 pytest 门禁（ISS-010B+ISS-077 后）是 **578**（计数以 `scripts/ci_pytest.sh` 与 CI 配置为准），变更测试数量必须用新任务和反例显式同步。本地复跑使用 macOS 系统 Bash：
...
```bash
/bin/bash scripts/ci_pytest.sh                       # ↔ pytest (arm64)，断言 578
