...
> 2026-09-19 生效范围校正：本文件保留 09-13 的目标设计与当时基线，不能用其“尚未实装/版本不一致”等旧现状描述接手。当前事实见 [ARCHITECTURE](../ARCHITECTURE.md)，队列见 [TASKS](../TASKS.md)。09-18 的 [DEC-022](../DECISIONS.md) 已将 Apple Developer ID / notarization / stapling 延期；下文这些门槛与 Apple 凭据方案作为未来恢复参考，不能阻断当前未签名内部候选，也不能勾为通过。双架构、updater 签名、下载 quarantine、升级恢复和用户批准公开不变。

状态：历史目标方案（受上述后续决定修订），尚未完成发行验证
基线：`main@8e7c07b`  
...
- 可移动版本标签的第三方 Actions；任何能读取 signing/updater secrets 或持有 write token 的 job，其全部 `uses:` 固定完整 commit SHA。

## 3. 架构与任务顺序

下图只表达发行主链；完整依赖、状态与可领取判断以 [TASKS](../TASKS.md) 索引为唯一来源。
...
2. arm64 与 Intel runner 各自锁定依赖、冻结 helper、构建 app/DMG/updater bundle；非特权 build job 仅有 `contents: read`；
...
6. 仅最终聚合/发布 job 取得最小 `contents: write` 并创建 draft Release，上传两个 DMG、两个 updater tar.gz、两个 `.sig`、checksums；
...
- 真实升级测试由 PM/CI 认证下载 private draft 资产，默认通过仅隔离测试机可达的本机/私网 HTTPS 与受信测试证书提供已签名更新包和 manifest；客户端不接触 GitHub token，测试后关闭测试源；若必须使用外部 staging，先将具体资产、URL、访问范围和自动失效时间交用户批准；
