...
#
# 本地等价命令（ISS-037 repair3 后 696 passed = 685 + 11(notices 许可证值级核对)；685 已含 040C +14 与 030A +16）：
...
#   - 通过数必须等于 EXPECTED_PYTEST_PASSED（默认 696）；计数变化必须
#     显式同步本默认值与任务证据，不允许静默漂移。
set -euo pipefail
cd "$(dirname "$0")/.."

expected="${EXPECTED_PYTEST_PASSED:-696}"
