...
- **安装界面未签名提示（2026-09-18 晚，用户决策，分支 `iss-009-install-hint`）**：用户明确签名/公证先不做（Folia 同模式），改为安装界面提示（[DEC-022](DECISIONS.md)）。交付：`scripts/build_dmg_background.py`（Pillow + 系统 Hiragino Sans GB 生成 660x480 背景，产物 `icons/dmg-background.png` 入库，日常构建零依赖）、`tauri.conf.json` `bundle.macOS.dmg`（背景/窗口尺寸/图标坐标；注：Tauri 2 字段为 `applicationFolderPosition` 非 `linkPosition`，`dmg` 段须在 `macOS` 下）、`verify_app_bundle.sh` 新增 (j) 段（DMG 挂载断言背景图 + app + Applications 链接在位）。实测：DMG 挂载 `.background/dmg-background.png` 41KB 在位；Finder 窗口截图目检绿色箭头对准 app→Applications、提示文案完整（未签名说明 + 右键打开/隐私与安全性两步放行）；**verify 全量 22/22 PASS**（第三次复跑确认；前两次各挂 1/4 段均为**运行期外部干扰**——一次是生产 7952 /health 瞬时抖动（404→000，单次 curl max-time 1s 采样），一次是 7953 被来历不明的外部 fathom helper 占用致壳复用（`c-helper-ready` pass 而 instance 不写、`i-exit-cleanup` 把 7953:200 记为外部既有监听为证），结束后均无残留进程；失败段全在只读 (j) 段之前，与本改动无关。(i) 段外部快照的单次采样脆弱性记为观察，暂不修）；ROADMAP M2 与 README 分发说明同步加注。
