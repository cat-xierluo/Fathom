...
- 默认只选当前阶段 READY，P0 优先，再按编号；当前明确用户指令优先。下一项默认 **ISS-019**，API/前端泳道为 **ISS-023**。ISS-026（UX 原型）、ISS-029（安装实验）允许 M0 提前验证，不等跨日观察；当前先完成 ISS-026 的第三轮视觉签名与复评，再开启下一波工程派发。
