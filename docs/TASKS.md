...
更新：2026-09-22。接手核对基线 `f8a64b18d82386f6dc0ba9cf92b111dfc674c148`（PR #148 已合并；代码事实仍为 #147，初始审查固定 `82b0616`）；本次 [方向复审](plans/2026-09-22-context-convergence-audit.md) 接续 [09-19 审查](plans/2026-09-19-pm-progression-audit.md)。执行前刷新远端与 worktree；索引是任务状态唯一来源，卡内有日期的旧派发、旧 READY、旧“下一步”均为历史证据。

## 当前接手摘要

- **当前目标**：收敛 M2 的 v0.3.0 可安装候选，补齐 M1 剩余原生体验门。M0 已有验收记录；M1/M2 仍未整体完成。已确认的五页 UI、R3 视觉及双形态图标继续使用；M4 智能解释、清理、扫描器重写不进入本轮。
- **已合并能力**：扫描/数据集/设置持久化、正式 UI 和 arm64 未签名包已有实现；近期新增 ISS-010B 后台注册桥（#131/#133）、ISS-016B 计划漂移检测与经确认重装（#139）、ISS-040B 更新插件/确认流/状态（#143）、ISS-078 候选登记工具（#140）、ISS-030A 隔离升级协议夹具（#146）。这些接线不再重新实施。ISS-001 已有两个有效定时日期与首份真实对比日报；扫描观察见原卡，不在摘要重复流水。
- **不能称为“只剩人工门”**：`updater_install` 仍直接调用插件安装，尚未接入 Fathom 的停写/一致备份/失败恢复协议，下载进度回调也为空；ISS-030A 是测试夹具，不能补足生产接线。ISS-010 的非默认运行根、唯一 owner 与睡眠/重启合同、ISS-037 声明缺项、ISS-041 双架构发行仍须收口。实现缺口和环境缺口分别记入原父卡。
- **最新合并与在途**：审查中 ISS-030A 已由原 PM 以 #146 → `8f3564d`、#147 → `31396a0` 合并收口，已保留其测试与证据，不再派发该夹具。当前接手先刷新开放 PR/worktree；ISS-079 上下文纠偏已由 #148 合并，本轮不接管运行中的 PM。
- **当前接续顺序**：①领取 ISS-080 同步本地/云端 pytest 计数；②在 ISS-040/030 原卡内明确生产更新协调的实现合同，先补安全接线及真实模块集成证据，再执行安装升级实测；③按条件补 ISS-028/077 前台验证与 ISS-002/003/008/009 的同候选实测，ISS-010/016 的 G8/G9 在隔离测试账户完成；④ISS-037 保持 PAUSED，恢复前须有明确重派决定；ISS-041 补原生 Intel 环境、双架构产物与 draft 流程，最后 ISS-030 实包恢复、ISS-033 外部验收。未备齐实现合同不自动派业务代码；没有合法 READY 时报告具体缺口，不以重复文档/夹具或视觉小修维持派发。
...
- **已确认取舍与证据**：DEC-022 仅延期 Apple 签名/公证；updater 签名、双架构、保留 quarantine 的真实下载及公开发布人工门保留。开发公钥/占位更新地址已入配置，生产更新源未启用。最新基线本地 pytest 配置 671、云端配置仍 655（差异归 ISS-080），既有记录前端 105、浏览器 39、cargo test 49；这是既有验证记录，本次未复跑。扫描超时根因仍未定位，成功次数不能证明资源竞争是原因。
...
- **实施边界**：先刷新最新 main；以已合并测试集合和本地门禁为依据，核对新增 16 项后的总数，再同步云端同口径值。ISS-030A 的测试与既有通过记录保留；本次文档审计只发现并登记，未改 CI 配置。
- **验收**：
  - [ ] 本地默认值、云端环境值与步骤名一致，保留与新增 16 项对应的可复核依据
  - [ ] 差异仅为计数/说明同步，不修改产品、测试语义、CI 启停或权限
  - [ ] 固定候选经独立复核；验证依据区分收集数量与实际通过数，不把 `--collect-only` 称为全量通过
- **证据**：`31396a0` 静态比对：`scripts/ci_pytest.sh` 默认 671；`.github/workflows/ci.yml` 环境值与步骤名 655。030A 运行记录见原卡/#146/#147；本卡尚未执行。
