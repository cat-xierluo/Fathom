...
- 增加单一运行配置与 schema v1：数据、报告、日志、扫描根、资源根和端口可完整隔离；旧 v0 库迁移前生成包含已提交 WAL 的一致 0600 备份，损坏、未来版本或结构不兼容的库会明确拒绝打开。
...
- 将 pytest 精确数量门禁同步为当前 168 项；默认数量和显式覆盖不一致都会非零退出，避免新增测试后本地与未来 CI 因旧计数持续假红。
