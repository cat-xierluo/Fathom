...
- **验收**：
  ...
  - [x] 独立单色 template tray 在 18/22pt 深浅菜单栏清晰，不直接缩小彩色 App Icon——`scripts/make_tray_icon.py` 重写为同构深度环距离场渲染（纯 stdlib），44×44 白色 template；实机菜单栏 22pt 浅色外观渲染正确（含「空闲」状态文字相邻），`lib.rs` 既有 `icon_as_template(true)` 由系统深浅反色；18pt 与深色模式实测 NOT_VERIFIED，并入 ISS-009 人工门清单
...
- **切片 2 实机验证与缺陷修复（2026-09-18 晚，PM，[PR #110](https://github.com/cat-xierluo/fathom/pull/110) squash 合并为 main `f349b78`）**：重跑打包链（`build_app.sh` rc=0，helper SHA256 `66d5e565…1667`，独立 reviewer 实算核对）后，以一次性脚本 `apps/desktop/src-tauri/verify-results/slice2-live/live_verify.sh`（gitignore 目录，日志/截图/instance 快照同目录留证）实机验证两场景，10 项断言全 PASS；独立 reviewer（agent_191cd1）5 项审查全 CONFIRMED（含 PyInstaller 6.22.3 `SourceDestAction` 分隔符语义、BSD grep BRE 交替真机复核、bash 3.2 `-n` 检查）**ACCEPT**，其指出的证据哈希笔误（`94e0dc05…`→`66d5e565…`）已修正：
  ...
  - **仍 NOT_VERIFIED（人工门，验收框 1/2 保持未勾）**：无 Python/Rust/Homebrew 新账户从安装（DMG）进入首扫与分布；tray 菜单手点退出（菜单构建有单测，实机手点未做）；断网首启；真实下载产物以开发者机拷贝以外的证据复核；正式深度环图标的 Launchpad/Spotlight 检索呈现与深色模式/18pt 小菜单栏下 tray 可辨性（ISS-045 资产已实测 DMG 窗口/Dock/22pt 浅色菜单栏，2026-09-19）；签名/公证（归 ISS-041）。
