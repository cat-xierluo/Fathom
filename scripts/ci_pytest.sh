...
#
# 本地等价命令（ISS-010B 后 571 passed = 553 + 18；全量复跑留 PM 收口，
# 18 = tests/test_launchd_autostart.py 22→40 的新增，其余测试文件未动）：
...
#   - 通过数必须等于 EXPECTED_PYTEST_PASSED（默认 571）；计数变化必须
#     显式同步本默认值与任务证据，不允许静默漂移。
set -euo pipefail
cd "$(dirname "$0")/.."

expected="${EXPECTED_PYTEST_PASSED:-571}"
