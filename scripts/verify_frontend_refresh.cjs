...
async function main() {
  ...
  try {
    ...
    const ringPersist = await page.$eval("#scan-badge .badge-ring svg", (el) => el.dataset.probe === "ring-node" && document.querySelector("#scan-badge .badge-ring") !== null, ring1)
      .catch(() => false);
    record("scan-badge-ring-persists-across-poll-ticks", ringPersist === true, `probe=${ringPersist}`);
