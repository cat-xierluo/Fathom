...
### 功能交付（Wave 1，PR #3/#4/#5，待合并）
- **扫描完成通知**（ISS-003）：扫描落库、日报写完后弹 macOS 通知，正文=今日 Top1 增长目录+剩余 GB；剩余低于 `config.FREE_ALERT_GB`（10GB）告警带声音；通知失败只记 `logs/notify.log`，绝不影响快照与日报
- **手动扫描状态持久化**（ISS-007）：三态（运行/完成/失败）写入 scan_runs 表，服务重启后状态可查；陈旧"运行中"超 1 小时自动收尾；API 返回向后兼容
- **tray 标题推送修复**（ISS-008）：根因——remote url 页面从未被授权 invoke `update_tray_status`，标题推送必失败；新增最小权限自定义权限修复；前端 invoke 失败可见并静默降级；Rust 空值/超长标题容错；tray 图标 2px 发丝线缺陷改 4px 实心环带
- 三交付由 GLM worker 并行实现（claude-code + glm-5.3），独立 reviewer ACCEPT（零阻塞发现），全量 32 passed；真机验收项（菜单栏三项、通知弹出）见任务卡 NOT_VERIFIED 清单
