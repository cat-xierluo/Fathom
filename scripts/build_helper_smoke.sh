...
REQ_RUNTIME="$ROOT/packaging/requirements-runtime.txt"
REQ_BUILD="$ROOT/packaging/requirements-runtime-build.txt"
...
if [ "$VENV_READY" != "yes" ] && [ "$AUTHORIZED" != "yes" ]; then
  ...
  log "工具/版本      : pyinstaller==${PIN_PYINSTALLER}（见 packaging/requirements-runtime-build.txt）"
  log "运行时依赖     : packaging/requirements-runtime.txt（fastapi==0.141.1, uvicorn==0.52.4）"
