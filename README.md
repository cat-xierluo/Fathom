...
目前是 **v0.3.0 开发版**，提供每日扫描内核、SQLite 快照、Markdown 日报、五页 Web 仪表盘和 Tauri 开发壳。还没有面向普通用户的完整安装包；数据可靠性与交互问题正在排期修复，详情见 [路线图](docs/ROADMAP.md) 与 [任务](docs/TASKS.md)。
...
- 当前已复现失败扫描覆盖当天有效数据、网页首扫误报失败、特殊路径解析和选择器过期，优先修复任务见 [审查记录](docs/plans/2026-09-12-project-review.md)。
...
当前未授予开源许可证，不作公开分发声明。依赖/资源许可清单及项目许可证选择由 ISS-037 完成，用户批准后再开放发布。
